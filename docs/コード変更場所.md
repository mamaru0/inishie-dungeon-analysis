...
- root配下に**crossdomain.xml**というファイルを作る。ファイルの中身は以下の通り。
```xml
<cross-domain-policy>
    <allow-access-from domain="*" />
</cross-domain-policy>
```
- Main 204行目: ```Security.loadPolicyFile .. ```に設定されたパスを、crossdomain.xmlのパスに変更します。
```as3
...
# 3. URLをローカルパスに変更
- Main 30行目: ```public var url:String .. ```に設定されたパスを、先程作成したrootフォルダーのパスに変更します。
```as3
