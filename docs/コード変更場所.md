...
- **inishie.cgi**というファイルを作る。ファイルの中身は空。AccessClass 341行目```this.loader.load(_loc2_);```でcgiファイルを読み込んだ後に、`Event.COMPLETE`というイベントを発生させる為だけに作ります。
- 本来はcgiでユーザーのアクセスを処理して、適切なデータを返します。
